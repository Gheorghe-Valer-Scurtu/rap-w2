class lhc_Travel definition inheriting from cl_abap_behavior_handler.
  private section.
    ...
    types:
      entity_create   type table for create zw2_r_travel_i,
      entity_update   type table for update zw2_r_travel_i,
      failed_entity   type table for failed   early zw2_r_travel_i,
      reported_entity type table for reported early zw2_r_travel_i.
...
*    lv_technical_user = 'DIFFERENT_USER'.
  ...
  method deductDiscount.
    ...
    data travels_update type table for update zw2_r_travel_i.

    data(keys_valid_discount) = keys.

    loop at keys_valid_discount assigning field-symbol(<key>)
      ...
      delete keys_valid_discount.
    endloop.

    check keys_valid_discount is not initial.

    read entities of zw2_r_travel_i in local mode
         entity Travel
         fields ( BookingFee )
         with corresponding #( keys_valid_discount )
         result data(travels).
    ...
    loop at travels assigning field-symbol(<travel>).
      ...
      data(discount_p) = keys_valid_discount[ key id %tky = <travel>-%tky ]-%param-discount_percent.
      percentage = discount_p / 100.
      data(reduced_fee) = <travel>-BookingFee * ( 1 - percentage ).

      append value #( %tky       = <travel>-%tky
                      BookingFee = reduced_fee ) to travels_update.
...
* EML - Entity Manipulation Language
  ...
  method setTravelNumber.
    ...
    select single from zw2_travel_i
  ...
  method validateCustomer.
    ...
    data customers type sorted table of /dmo/customer with unique key client
                                                                      customer_id.
    ...
    loop at travels into data(travel).
      ...
      elseif not travel-CustomerID is initial and not line_exists( valid_customers[ customer_id = travel-CustomerID ] ) .
